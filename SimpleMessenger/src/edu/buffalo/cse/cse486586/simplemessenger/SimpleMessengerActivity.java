...
public class SimpleMessengerActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		/*
		 * Allow this Activity to use a layout file that defines what UI
		 * elements to use. Please take a look at res/layout/main.xml to see how
		 * the UI elements are defined.
		 * 
		 * R is an automatically generated class that contains pointers to
		 * statically declared "resources" such as UI elements and strings. For
		 * example, R.layout.main refers to the entire UI screen declared in
		 * res/layout/main.xml file. You can find other examples of R class
		 * variables below.
		 */
		 ...
		 * 
		 * This is another example of R class variables. R.id.edit_text refers
		 * to the EditText UI element declared in res/layout/main.xml. The id of
		 * "edit_text" is given in that file by the use of
		 * "android:id="@+id/edit_text""
		 */
		final EditText editText = (EditText) findViewById(R.id.edit_text);

		/*
		 * Register an OnKeyListener for the input box. OnKeyListener is an
		 * event handler that processes each key event. The purpose of the
		 * following code is to detect an enter key press event, and create a
		 * client thread so that the client thread can send the string in the
		 * input box over the network.
		 */
		editText.setOnKeyListener(new OnKeyListener() {
			...
			public boolean onKey(View v, int keyCode, KeyEvent event) {
				if ((event.getAction() == KeyEvent.ACTION_DOWN)
					...
					localTextView.append("\t" + msg); // This is one way to
														// display a string.
	...
	private class ServerTask extends AsyncTask<ServerSocket, String, Void> {
		...
		protected void onProgressUpdate(String... strings) {
			...
			Log.d("", "MESSAGE Saved on file before:" + strReceived + "$");
			String string = strReceived + "\n";
			Log.d("", "MESSAGE Saved on file after:" + strReceived + "$");
